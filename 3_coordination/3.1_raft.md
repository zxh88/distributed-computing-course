...
在T0(TERM2)时刻，S1当选leader，把某个日志项复制到了S1,S2两个节点上后宕机了，然后在T1(TERM3)时刻，S5获得了S3,S4的投票当选为新的leader，并在本机添加了一个新的日志项后宕机了，接下来在T2(TERM4)时刻，S1重新当选leader，接收了一个新的日志项，并把之前TERM2接收到的日志项继续复制到了S3上，此时TERM2上的这个日志项因为已被大部分节点接收，所以可以提交(commit)并应用(apply)了，但是此时S1又宕机了，S5因为最近一个日志项的term比较大所以重新当选leader

针对这种情况，raft给出的解决方案是：新leader不能只通过已复制成功的份数来确定来自旧leader的日志项能不能提交，新leader只能通过计数来标记自己复制的日志能否提交，根据之前提到的`Log Matching`性质，只要当前leader接收的日志在集群可以提交了，则旧leader的日志也自然而然的变为可提交的了。(如果不采用这条方案，`Leader Completeness`性质就得不到保证了：之前leader提交的日志被之后的leader覆盖了)
